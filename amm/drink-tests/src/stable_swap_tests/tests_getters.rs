...
fn test_02(mut session: Session) {
    let initial_reserves = vec![10000000 * ONE_USDT, 10000000 * ONE_USDC];
    let initial_supply: Vec<u128> = initial_reserves.iter().map(|amount| amount * 10).collect();
    let amp_coef = 10_000u128;
    let trade_fee = 2_500_000u32;
    let protocol_fee = 200_000_000u32;
    ...
    // make a swap to the reserves are not equal anymore
    _ = stable_swap::swap_exact_in(
        &mut session,
        stable_swap,
        BOB,
    ...
    let (share_burn_1, fee) =
        stable_swap::get_burn_liquidity_for_amounts(&mut session, stable_swap, withdraw_amounts)
            .expect("Should compute");
    // both methods should require burning the same amount lpt
    assert_eq!(share, share_burn_1, "Shares amounts mismatch");
    // because `withdraw_amounts` are balanced, fee should be 0
    assert_eq!(fee, 0, "Fee should be 0");
}
